...
app.use(session({
    secret: 'raadi-secure-emperor-falcon-key-20 TEXT NOT NULL, price REAL NOT NULL, oldPrice REAL, discount INTEGER DEFAULT 0, color TEXT, features TEXT, stock INTEGER DEFAULT 0, image TEXT, createdAt TEXT)`);
    ...
    db.run(`CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, userId INTEGER, sender TEXT, text TEXT, isAdmin INTEGER DEFAULT 0,26',
    resave: false,
    saveUninitialized: true,
    cookie: { 
        maxAge: 1000 * 60 * 60 * 24 * 7,
        secure: false, // لضمان ثبات جلسات الدخول على Render بدون تعقيدات شهادات الأمان
        sameSite: 'lax'
    }
}));
app.use(express.json isRead INTEGER DEFAULT 0, timestamp TEXT, date TEXT)`);
    ...
    // إدخال الإعدادات والألوان الافتراضية
    db.get("SELECT COUNT(*) as count FROM());
app.use(express.urlencoded({ extended: true }));
app.use(express.static(path.join(__dirname, ' categories", (err, row) => {
        if (row && row.count === 0) {
            ['هواتف', 'عطور', 'إكسسوارات', 'إلكترونيات'].forEachpublic')));
...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT DEFAULT 'user', phone TEXT, address TEXT, avatar TEXT, createdAt TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY AUTOINCREMENT, userId INTEGER, orderNumber TEXT UNIQUE, customer TEXT, email TEXT, phone TEXT, address TEXT, country TEXT,(cat => {
    ...
    db.get("SELECT COUNT(*) as count FROM products", (err, row) => {
        if (row && row.count === 0) {
            const sample = [
                ...
                items TEXT, subtotal REAL, discount INTEGER, discountAmount REAL, shipping REAL, total REAL, status TEXT, date TEXT, dateFormatted TEXT)`);
    ...
    db.run(`CREATE TABLE IF NOT EXISTS groups (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE NOT NULL, members TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS coupons ['سامسونج S24 الترا', 'هواتف', 4940, 5200, 12, 'رمادي تيتانيوم', 'قلم ذكي مدمج وشاشة عالية السطوع بمواصفات جبارة', 8, 'https://picsum.photos/id/1/300/300'],
                ...
                (id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT UNIQUE, value INTEGER, createdAt TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)`);
    db.run(`CREATE TABLE IF NOT EXISTS reviews (id INTEGER PRIMARY KEY AUTOINCREMENT, productId INTEGER, userId INTEGER, userName TEXT, rating INTEGER, comment TEXT, date TEXT)`); ['عطر بلو دي شانيل', 'عطور', 4140, 4600, 10, 'كحلي داكن', 'عبق فريد يدوم طويلاً لأصحاب الفخامة والروعة الملوكية', 6, 'https://picsum.photos/id/2/300/300']
    ...
    db.get("SELECT COUNT(*) as count FROM categories", (err, row) => {
        if (row && row.count === 0) {
            ...
            stmt.finalize();db.run("INSERT INTO categories (name) VALUES (?)", [cat]);
            });
        }
    });
    db.get("SELECT COUNT(*) as count FROM products", (err, row) => {
    ...
    db.get("SELECT COUNT(*) as count FROM settings", (err, row) => {
        if (row && row.count === 0) {
            ...
            db.run("INSERT INTO settings (key, value) VALUES ['سامسونج S24 الترا', 'هواتف', 4940, 5200, 12, 'رمادي تيتانيوم', 'قلم ذكي مدمج وشاشة عالية السطوع بمواصفات جبارة', 8, 'https://picsum.photos/id/1/300/300'],
                ('voiceGender', 'female')");
            db.run("INSERT INTO settings (key, value) VALUES ('voiceText', 'أهلاً وسهلاً بك في متجر الرعدي أونلاين الفاخر')");
            db.run("INSERT INTO settings (key, value) VALUES ('primaryColor', '#0f2c4a')");
            db.run("INSERT INTO settings (key, value) VALUES ('accentColor', '#d4af37')");
            db.run("INSERT INTO settings (key, value) VALUES ('bgColor', '#0a ['سماعة إيربودز برو', 'إكسسوارات', 899, 1099, 18, 'أبيض ناصع', 'تقنية نشطة لعزل الضوضاء المحيطة وصوت محيطي فخم', 20, 'https://picsum.photos/id/3/300/300'],
            ...
            const stmt = db.prepare("INSERT INTO products (name, category, price192f')");
...
const run = (sql, params = []) => new Promise((res, rej) => db.run(sql, params, function(err), oldPrice, discount, color, features, stock, image, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            sample.forEach(p => stmt.run(...p, new Date().toISOString()));
            stmt.finalize();
        }
    });
    ...
    db.get("SELECT COUNT(*) as count FROM settings", (err, row) => {
        if (row && row.count === 0) {
            db.run("INSERT INTO settings (key, value) VALUES ('domesticShipping', '1 { err ? rej(err) : res({ lastID: this.lastID }); }));
...
app.post('/api/categories', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح'5')");
            db.run("INSERT INTO settings (key, value) VALUES ('internationalShipping', '50')");
            db.run("INSERT INTO settings (key, value) VALUES ('whatsappNumber', '+967781723532')");
            db.run("INSERT INTO settings (key, value) VALUES ('marqueeText', '🦅 أهلاً ومرحباً بكم في متجر الرعدي أونلاين الفاخر - شحن مجاني للمشتريات فوق 300 ريال')");
    ...
    try { await run("INSERT INTO categories (name) VALUES (?)", [name.trim()]); res.json({ success: true }); } catch { res.statusrun("INSERT INTO settings (key, value) VALUES ('voiceText', 'أهلاً وسهلاً بك في متجر الرعدي أونلاين الفاخر')");
            db.run("INSERT INTO settings (key, value) VALUES ('returnPolicy', '1. لا يلزم المتجر إعادة المنتج بعد 3 أيام من الاستلام.\\n2. لا يمكن إرجاع المنتج في حال وجود عيب ناتج عن سوء الاستخدام.\\n3. لا يلزم المتجر رد النقود نقداً ويمكن للعميل استبدال السلع بمنتج آخر بنفس القيمة.')");
        }
    });
});
...
const query = (sql, params = []) => new Promise((res, rej) => db.all(sql, params, (err, rows) => err ? rej(err) : res(rows)));
const get = (sql, params = []) => new Promise((res, rej) => db.get(sql, params, (err, row) => err ? rej(500).json({ error: 'القسم مضاف مسبقاً' }); }
});
app.delete('/api/categories/:name', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { await run("DELETE FROM categories WHERE name = ?", [req.params.name]); await run("DELETE FROM products WHERE category = ?", [req.params.name]); res.json({(err) : res(row)));
const run = (sql, params = []) => new Promise((res, rej) => db.run(sql, params, function(err) { err ? rej(err) : res({ lastID: this.last success: true }); } catch { res.status(500).json({ error: 'حدث خطأ' }); }
});

app.get('/api/products', async (req, res) => {
    try { res.json(await query("SELECT * FROM products ORDER BY id DESC")); } catch { res.status(ID }); }));

// ---------------------------- واجهات التطبيق البرمجية (APIs) ----------------------------
app.get('/api/categories', async (req, res) => {
    try { res.json((await query("SELECT name FROM categories")).map(r => r.name)); } catch { res.status(500).json([]); }
});
app.post('/api/categories', async (req, res) => {500).json([]); }
});
app.post('/api/products', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    const { name, category, price, oldPrice, discount, color, features, stock, image } = req.body;
    try { await run(`INSERT INTO
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    const { name } = req.body;
    if products (name, category, price, oldPrice, discount, color, features, stock, image, createdAt) VALUES (!name) return res.status(400).json({ error: 'الاسم مطلوب' });
...
app.get('/api/products', async (req, res (?,?,?,?,?,?,?,?,?,?)`, [name, category, price, oldPrice||0, discount||0, color, features, stock||0, image, new Date().toISOString()]); res.json({ success: true }); } catch { res.status(500).json({ error: 'حدث خطأ' }); }
});
...
app.post('/api/products', async (req, res) => {
    ...
    const { name, category
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { await run("DELETE FROM products WHERE id = ?", [req.params.id]); res.json({ success: true }); } catch { res.status(500, price, oldPrice, discount, color, features, stock, image } = req.body;
...
app.post('/api/settings', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير});
app.put('/api/products/:id', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    const { name, price, oldPrice, discount مصرح' });
...
});, stock, image, category, color, features } = req.body;
...
// مسارات المصادقة وتحديث الحسابات للعملاء والمدير
app.post('/api/register', async (req, res) => {
    const { name, email, password, phone, address } = req.body;
    try {
        if (await get("SELECT id FROM users WHERE email = ?", [email])) returnapi/products/:id', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { await run("DELETE FROM products WHERE id = ?", [req.params.id]); res.json({ success: true }); } catch { res.status(500).json({ error: 'حدث خطأ' }); }
});

// مراجعات وتقييمات المنتجات المباشرة
app.get('/api/products/:id/reviews', async (req, res) => {
    try { res.json(await query("SELECT * FROM reviews WHERE productId = ? ORDER BY id DESC", [req.params.id])); } catch { res.status(5 res.status(400).json({ error: 'البريد مسجل مسبقاً' });
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        if (!u || !bcrypt.compareSync(password, u.password)) return res.status(4000).json([]); }
});
app.post('/api/products/:id/reviews', async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ error: 'سجل أولاً' });
    const { rating, comment } = req.body;
    try {
        await run("INSERT INTO reviews (productId, userId, userName, rating, comment, date) VALUES (?, ?, ?, ?, ?, ?)", 
            [req.params.id, req.session.userId, req.session.user.name, rating, comment, new Date().toLocaleDateString('ar-EG')]);
        res.json({ success: true1).json({ error: 'بيانات الدخول غير صحيحة' });
...
app.get('/api/settings', async (req, res) => {
    try { const rows = await query("SELECT key, value FROM settings"); const s = {}; rows.forEach(r => s[r.key] = r.value); res.json(s); } catch { res.status(500).json({});, email, password, phone, address, avatar } = req.body;
    try {
        let passUpdate = '';
...
app.post('/api/settings', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { for (const [k, v] of Object.entries(req.body)) { await run("UPDATE settings SET value = ? WHERE key = ?", [v.toString(), k]); } res.json({ success: true }); } catch { res.status(50me', (req, res) => req.session.userId ? res.json(req.session.user) : res.status(401).json({ error: 'غير مسجل' }));
app.post('/api/logout', (req, res) => { req.session.destroy(); res.json({ success: true }); });

// الطلبات
app.post('/api/orders', async (req, res) => {
    if (!req.session.userId) return res.status0).json({ error: 'حدث خطأ' }); }
});
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        await run("INSERT INTO users (name, email, password, phone, address, role, avatar(401).json({ error: 'سجل دخولك أولاً' });
    ...
    try {
        ...
        await run(`INSERT INTO orders (userId, orderNumber, customer, email, phone, address, country, items, subtotal, discount, discountAmount, shipping, total, status, date, dateFormatted) VALUES (?,?,?,?,?,?,?,?,?,?,?,, createdAt) VALUES (?,?,?,?,?,?,?,?)", [name, email, bcrypt.hashSync(password, 10), phone, address, 'user', '', new Date().toISOString()]);
        res.json({ success: true });
    } catch { res.status(500).json({ error: 'خطأ بالسيرفر' }); }
});
app.post('/api/login', async (req, res) => {
    const { email, password } = req.body;
    try {
        const u = await get("SELECT * FROM users WHERE?,?,?,?,?)`, 
        ...
        for (const item of items) {
            ...
            if (p) await run("UPDATE products SET stock = ? WHERE id = ?", email = ?", [email]);
        if (!u || !bcrypt.compareSync(password, u.password)) return res.status(401).json({ error: 'بيانات الدخول غير صحيحة' });
        req.session.userId = u.id;
        req.session.user = { id: u.id, name: u.name, email: u.email, role: u.role, phone: u.phone [Math.max(0, p.stock - item.quantity), item.id]);
        }
        res.json({ success: true });
    } catch { res.status(500).json({ error: 'حدث خطأ' }); }
});
app.get('/api/my-orders', async (req, res) => {
    if (!req.session.userId) return res.status(401).json([]);, address: u.address, avatar: u.avatar };
        res.json({ success: true, user: req.session.user });
    } catch { res.status(500).json({ error: 'خطأ بالسيرفر' }); }
});
app.post('/api/profile/update', async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ error
...
app.put('/api/orders/:id/status', async (req, res) => {
    if (!req.session.userId || req.session.user.role: 'غير مسجل' });
    ...
    try {
        ...
        req.session.user = { id: req.session.userId, name, email, role: req.session.user.role, phone, address, avatar };
        res.json({ success: true, user: req.session.user });
    } catch { res.status(500).json({ error !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
...
app.post('/api/groups', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { await run("INSERT INTO groups: 'خطأ في التعديل' }); }
});
...
app.post('/api/orders', async (req, res) => {
    ...
    try {
        ...
        await run(`INSERT INTO orders (userId, orderNumber, customer, email, phone, address, country, items, subtotal, discount, discountAmount, shipping, total, status, date, dateFormatted) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, 
            [req.session.userId, num, customer, email, phone, address, country, JSON.stringify(items), subtotal, discount, discountAmount, shipping, total, 'قيد المعالجة', new Date().toISOString(), new Date().toLocaleDateString('ar (name, members) VALUES (?, ?)", [req.body.name, JSON.stringify([])]); res.json({ success: true }); } catch { res.status(500).json({ error: 'حدث خطأ' }); }
});
...
app.get('/api/mysession.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
...
app.get('/api/coupons', async (req, res) => {
    try { res.json(await query("SELECT code, value FROM coupons    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json([]);
    try { const list = await query("SELECT * FROM orders ORDER BY id DESC"); list.forEach(o => o.items = JSON.parse(o.items)); res.json(list); } catch { res.status(500).json([]); }
});
app.put('/api/orders/:id/status', async (req, res) => {
    if (!req.session.userId")); } catch { res.status(500).json([]); }
});
...
app.delete('/api/coupons/:code', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { await run("UPDATE orders SET status = ? WHERE id = ?", [req.body.status, req.params.id]); res.json({ success: true }); } catch { res.status(500).json({ error: 'خطأ' }); }
});
...
// الدردشة والرسائل المباشرة});
app.post('/api/groups', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { await run("INSERT INTO groups (name, members) VALUES (?, ?)", [req.body.name, JSON.stringify([])]); res.json({ success: true الفورية
app.get('/api/messages', async (req, res) => {
    ...
    try {
        ...
        res.json(list);
    } catch { res.status(5 }); } catch { res.status(500).json({ error: 'حدث خطأ' }); }
});
app.put('/api/groups/:id', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { await run("UPDATE groups SET members = ? WHERE id = ?", [JSON.stringify(req.body.members), req.params.id]); res.json({ success: true }); } catch { res.status(500).json({ error: 'خطأ' }); }
});
app.delete('/api/groups/:id', async (req, res) => {00).json([]); }
});
...
app.get('/api/users', async ( {
    ...
    try { 
        await run(`INSERTreq, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json([]);
    try { res.json(await query("SELECT id, name, email, role, phone, address, avatar, createdAt FROM users WHERE role != 'admin INTO messages (userId, sender, text, isAdmin, isRead, timestamp, date) VALUES (?,?,?,?,?,?,?)`, 
...
app.get('/api/stats', async (req, res) => {
    ...
    try {
        ...
        const p = await get("SELECT COUNT(*) as count FROM products");'")); } catch { res.status(500).json([]); }
});
app.get('/api/coupons', async (req, res) => {
...
app.post('/api/coupons', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    try { await run("INSERT INTO coupons (code, value, createdAt) VALUES (?,
        ...
        const todayRev = await get("SELECT SUM(total) as sum FROM orders WHERE date LIKE ?",?,?)", [req.body.code.toUpperCase(), req.body.value, new Date().toISOString()]); res.json({ success: true }); } catch { res.status(500).json({ error: 'موجود مسبقاً' }); }
});
app.delete('/api/coupons/:code', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res [todayISO + '%']);
...
app.get('/', (req, res) => res.status(403).json({ error: 'غير مصرح' });
    try { await run("DELETE FROM coupons WHERE code = ?", [req.params.code]); res.json({ success: true }); } catch { res.status(500).json({ error: 'خطأ' }); }
});

// الدردشة المباشرة
app.get('/api/messages', async (req, res) => {
    if (!req.sendFile(path.join(__dirname, 'public', 'index.html')));
